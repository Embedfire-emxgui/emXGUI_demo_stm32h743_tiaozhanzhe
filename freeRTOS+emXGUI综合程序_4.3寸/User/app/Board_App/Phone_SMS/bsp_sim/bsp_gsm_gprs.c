...
/**
  * @brief  GUI使用特殊函数,保存电话号码数据指针返回
	* @param  none 					    
  * @retval 0                 0表示失败
  * @retval int型号码         
  */
uint16_t gsm_ReadSaveAdd(void)
{
...
/**
  * @brief  保存短信并发送（支持中英文,中文为GBK码）
	* @param  num  					    短信地址
	* @param  smstext         	保存的信息内容指针
	* @param  str	              保存短信内容(unicode编码格式的字符串)
  * @retval 0                 0表示失败
  * @retval 1                 表示成功读取到短信，该短信未读（此处是第一次读，读完后会自动被标准为已读）
  * @retval 2                 表示成功读取到短信，该短信已读
  */
uint16_t gsm_save_sms(char *num,char *smstext)
{
  char ucsbuff[160];
	char CMD_buf[48]={0};
  uint16_t Add;
  ...
  {
    //英文
    GSM_TX("AT+CSCS=\"GSM\"\r");     //"GSM"字符集
    GSM_DELAY(100);
    
    GSM_TX("AT+CMGF=1\r");           //文本模式
    GSM_DELAY(100);
    
		sprintf(CMD_buf,"AT+CMGW=\"%s\"\r",num);
    GSM_TX(CMD_buf);  //电话号码
    GSM_DELAY(100);
    memset(CMD_buf,'\0',40);
		
		sprintf(CMD_buf,"%s",smstext);
    GSM_TX(CMD_buf);            //短信内容
    GSM_DELAY(1); 
		memset(CMD_buf,'\0',40);
  }
  ...
  {
    //中文
    GSM_TX("AT+CSCS=\"UCS2\"\r");    //"UCS2"字符集
    GSM_DELAY(100);
    
    GSM_TX("AT+CMGF=1\r");           //文本模式
    GSM_DELAY(100);
    
    GSM_TX("AT+CSMP=17,167,0,8\r");  //
    GSM_DELAY(100);
    
    gsm_gbk2ucs2hex(ucsbuff,num);
		sprintf(CMD_buf,"AT+CMGW=\"%s\"\r",ucsbuff);
    GSM_TX(CMD_buf);  //UCS2的电话号码(需要转成 ucs2码)
    GSM_DELAY(100);
    memset(CMD_buf,'\0',40);
		
    gsm_gbk2ucs2hex(ucsbuff,smstext);
		sprintf(CMD_buf,"%s",ucsbuff);
    GSM_TX(CMD_buf);          //UCS2的文本内容(需要转成 ucs2码) 
    memset(CMD_buf,'\0',40);		
  }
  
  GSM_CLEAN_RX();         // 清空了接收缓冲区数据
  GSM_USARTx->RDR=(u32)0x1A;		// 发送十六进制数：0X1A,信息结束符号
  GSM_DELAY(200);
  ...
  if (Add == 0) 
	...
	sprintf(CMD_buf,"AT+CMSS=%d\r",Add);
  GSM_TX(CMD_buf);  //发送短信
...
/**
  * @brief  读取短信内容
	* @param  messadd  					短信地址
	* @param  num         			保存发件人号码(unicode编码格式的字符串)
	* @param  str	              保存短信内容(unicode编码格式的字符串)
  * @retval 0                 0表示失败
  * @retval 1                 表示成功读取到短信，该短信未读（此处是第一次读，读完后会自动被标准为已读）
  * @retval 2                 表示成功读取到短信，该短信已读
...
/*------------- 读取短信内容 ----------------------------*/
	sprintf(cmd,"AT+CMGR=%d\r",messadd);	
	if(gsm_cmd(cmd,"+CMGR:",100) != GSM_TRUE)
	{
		return 0;
	}
	redata = GSM_RX(len);   //接收数据
