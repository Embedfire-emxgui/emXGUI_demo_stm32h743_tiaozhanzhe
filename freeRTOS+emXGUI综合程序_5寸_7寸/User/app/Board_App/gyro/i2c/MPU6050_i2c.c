...
/**
  ...
  * @retval  无
  */
static uint32_t I2C_ReadBytes(uint8_t ClientAddr,uint8_t* pBuffer, uint16_t NumByteToRead)
{
...
/**
  ...
  * @retval  无
  */
static uint32_t I2C_WriteBytes(uint8_t ClientAddr,uint8_t* pBuffer,  uint8_t NumByteToWrite)
{
	uint16_t m;	
...
/**
  ...
  * @retval  
  */
static uint32_t Sensor_write(uint8_t reg_add,uint8_t reg_dat)
{ 
	/*　第0步：发停止信号，启动内部写操作　*/
	i2c_Stop();
	/* 第1步：发起I2C总线启动信号 */
...
/**
  ...
  * @retval  
  */
static uint32_t Sensor_Read(uint8_t reg_add,unsigned char* Read,uint8_t num)
{ 
	/* 第0步：发停止信号，启动内部写操作　*/
	i2c_Stop();
	/* 第1步：发起I2C总线启动信号 */
